...
/* GET home page. */
router.get('/api/v1/dog', function(req, res, next) {
    const Day = new Date();
    const date =
      Day.getFullYear() + "-" + (Day.getMonth() + 1) + "-" + Day.getDate();
    try{
    let avalible_options = [
        "fact",
        "image",
        "both"
    ]
    ...
    if(option == "both" || option == "image") {
        let bears = fs.readdirSync('./public/img/dog');
        image = bears[Math.floor(Math.random() * bears.length)];
        image = req.protocol + '://' + req.get('host') + '/img/dog/' + image;
    }

    let fact
    if(option == "both" || option == "fact") {
        let facts = fs.readFileSync('./public/json/dogs-fact.json', 'utf8');
